...
/* 使用 DMA 分块发送颜色数据 */
void ST7789_DMA_PushColors(uint16_t *color, uint32_t size){
    if (size == 0) return;

    uint32_t Sending_Size = size * 2; /* 字节数 */
    uint32_t Buffer_Size = (Sending_Size < BURST_MAX_SIZE) ? Sending_Size : BURST_MAX_SIZE;
    uint32_t Sending_in_Block = Sending_Size / Buffer_Size;
    uint32_t Remainder_from_block = Sending_Size % Buffer_Size;
    ...
    uint32_t offset_words = 0; /* 偏移（以 16-bit 字为单位） */

    for(uint32_t b = 0; b < Sending_in_Block; ++b)
    {
        uint8_t *pbuf = (uint8_t*)(color + offset_words);
        if (HAL_SPI_Transmit_DMA(&hspi1, pbuf, Buffer_Size) != HAL_OK) {
            /* 失败处理：中止并退出 */
            HAL_SPI_Abort(&hspi1);
            break;
        }
        /* 等待本次 DMA 完成（简单轮询 + 超时保护） */
        uint32_t start = HAL_GetTick();
        while(HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY) {
            if ((HAL_GetTick() - start) > 1000) { /* 1s 超时，可按需调整 */
                HAL_SPI_Abort(&hspi1);
                break;
            }
        }
        offset_words += Buffer_Size / 2;
    }
    ...
    {
        uint8_t *pbuf = (uint8_t*)(color + offset_words);
        if (HAL_SPI_Transmit_DMA(&hspi1, pbuf, Remainder_from_block) == HAL_OK) {
            uint32_t start = HAL_GetTick();
            while(HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY) {
